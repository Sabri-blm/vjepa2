...
def main(args, resume_preempt=False):
    ...
    start_epoch = 0
    load_checkpoint_pred = False
    # -- load training checkpoint
    if load_checkpoint_pred and os.path.exists(latest_path):
        (
            encoder,
            predictor,
            target_encoder,
    ...
    # -- TRAINING LOOP
    for epoch in tqdm(range(start_epoch, num_epochs), total=num_epochs, desc="Finetuning"):
        ...
        for itr in range(ipe):
            ...
            def load_clips():
                ...
                rainfall_transitions = sample[1].to(device, dtype=torch.float, non_blocking=True)  # [B T-1 1]

                return clips, rainfall_transitions

            clips, rainfall_transitions = load_clips()
            ...
            def log_stats():
                ...
                if (itr % log_freq == 0) or (itr == ipe - 1) or np.isnan(loss) or np.isinf(loss):
                    ...
                    wandb.log({
                        ...
                        "iteration": itr,
                        "loss": loss_meter.avg,
                        "teacher_forcing loss": jloss_meter.avg,
                        "rollout loss": sloss_meter.avg,
                        "lr": _new_lr,
