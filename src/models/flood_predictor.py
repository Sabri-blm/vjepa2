...
class VisionTransformerPredictorFlood(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.meta_encoder = nn.Linear(meta_embed_dim, predictor_embed_dim, bias=True)
    ...
    def forward(self, x, meta=None):
        ...
        meta = self.meta_encoder(meta).unsqueeze(2)
        x = x.view(B, T, self.grid_height * self.grid_width, D)  # [B, T, H*W, D]

        x = torch.cat([meta, x], dim=2).flatten(1, 2)  # [B, T*(H*W+1), D]
...
def vit_flood_predictor(**kwargs):
    model = VisionTransformerPredictorFlood(
        mlp_ratio=4,
        qkv_bias=True,
